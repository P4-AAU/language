...
let type_error (loc : location) msg =
  let (pos, _) = loc in
  failwith (Printf.sprintf "%s:%d:%d: %s"
...
let check_size ty expr =
  let opt_n = match expr.expr_node with
    | Ecst (Cint n) -> Some n
...
let rec infer_expr env expr =
  ...
  | Eident id ->
    (try Env.find id.id env
     with Not_found -> type_error id.loc (Printf.sprintf "unknown variable: %s" id.id))
  ...
  | Ebufread (buf_expr, idx_expr) ->
    ...
    in
    let idx_type = infer_expr env idx_expr in
    if not (is_int_type idx_type) then
...
and check_return_in_stmt env loc stmt =
  ...
  | Sblock stmts ->
    List.fold_left (fun (has_return, return_type) s ->
...
and check_stmt env stmt =
  ...
  | Sassign (id, expr) ->
    let t =
      try Env.find id.id env
      with Not_found -> type_error id.loc (Printf.sprintf "Variable %s not found" id.id)
    in
    let te = infer_expr env expr in
    if not (types_compatible t expr te) then
      type_error expr.expr_loc (Printf.sprintf "type mismatch in assign '%s'" id.id);
    check_size t expr;
    env
  ...
  | Sdefine (id, ty, expr) ->
    if Env.mem id.id env then
      type_error id.loc (Printf.sprintf "Variable %s is already defined" id.id);
    let te = infer_expr env expr in
    if not (types_compatible ty expr te) then
      type_error id.loc (Printf.sprintf
  ...
  | Sif (cond, thn, els) ->
    ...
    if tc <> Tbool then
      type_error cond.expr_loc "expression does not evaluate to bool";
  ...
  | Sfunc (func_name, func_type, params_list, func_body) ->
    ...
    let function_scope = List.fold_left (fun local_env (param_name, param_type) ->
      if Env.mem param_name.id local_env then
    ...
    if actual_return_type <> func_type then
      type_error func_name.loc "return type does not match function type";
  ...
  | Sfor (iter_name, input, body) ->
    if Env.mem iter_name.id env then
      type_error iter_name.loc "iterator name already defined";
