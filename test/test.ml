...
let test_valid () =
  ...
  | exception exn -> print_endline ("Unexpected error: " ^ Printexc.to_string exn)
;;

let test_invalid () =
  match parse "{ x === 5; }" with
  | _ -> print_endline "FAIL: should have errored"
  | exception exn -> print_endline ("Rejected as expected: " ^ Printexc.to_string exn)
;;

let test_buffer_declare () =
  match parse "{ buffer buf<int32, 3> = [1, 2, 3]; }" with
  | _ -> print_endline "OK: buffer declaration with init values"
  | exception exn -> print_endline ("FAIL: " ^ Printexc.to_string exn)
;;
...
let test_bufread () =
  match
    parse "{ buffer buf<int32, 3> = [1, 2, 3]; define x of int32 = bufread(buf, 0); }"
  with
